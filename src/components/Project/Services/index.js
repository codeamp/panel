...
export default class Services extends React.Component {
              ...
              <div className={styles.drawerBody}>
                <Grid container spacing={24} className={styles.grid}>
                  ...
                  <Grid item xs={12}>
                      ...
                      {this.renderDrawerAdvancedSettings()}
  ...
  renderConfirmDeleteDialog(services) {
    if (services.entries[this.form.values()['index']]) {
      return (
        <Dialog open={this.state.showConfirmDeleteDialog}>
          <DialogTitle>{"Are you sure you want to delete " + services.entries[this.form.values()['index']].name + "?"}</DialogTitle>
          <DialogContent>
            <DialogContentText>
  ...
  renderPaginator(loading, data, error) {
    if (loading || error){
      return (
        <Loading />
      )
    ...
    let items = data.project.services.entries
    return (
      <Table>
        <TableHead>
          <TableRow>
        ...
        <TableBody>
          {items.map( (paginatedItem, index) => {
            return this.renderPaginatedItem(paginatedItem, index)
          })}
        </TableBody>      
      </Table> 
    )
  ...
  onPaginatorPageChanged(page, limit){
    this.props.history.push({
      pathname: this.props.location.pathname,
  ...
  renderPaginatedItem(service, index) {
    return (
      <TableRow
        hover
  ...
  onDataLoaded(queryData) {
    ...
    let serviceSpecs = queryData.serviceSpecs
      ...
      <div>
        <Paper className={styles.tablePaper}>
          ...
          <Paginator 
            ...
            render={this.renderPaginator.bind(this)}
            renderControls={this.renderPaginatorControls.bind(this)}
            countPages={ (data, limit) => {return Math.ceil(data.project.services.count / limit)} }
            onCompleted={this.onDataLoaded.bind(this)}
            onError={this.onDataError.bind(this)}
            onPageChanged={this.onPaginatorPageChanged.bind(this)}
            {...this.props}
          />
